...
router.route('/')
    .get(getProducts)
    .post(authorize('PRODUCT_ADMIN'), upload.array('images', 5), logAction('CREATE_PRODUCT', 'PRODUCT'), createProduct);

router.route('/:id')
    .get(getProduct)
    .put(authorize('PRODUCT_ADMIN'), upload.array('images', 5), logAction('UPDATE_PRODUCT', 'PRODUCT'), updateProduct)
